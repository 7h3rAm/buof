...
class BUOF:
  ...
  def opcode_offset(self, eipvalue):
    bufsize, maxbufsize = 10, 10000
    while True:
      fuzbuf = self.pcreate(bufsize)
      try:
  ...
  def opcode_badchars(self, chars):
...
def main(args):
  ...
  if args.opcode == "fuzz":
    ...
    buof.opcode_offset(args.eipvalue)

  elif args.opcode == "badchars":
    buof.opcode_badchars(args.badchars)
...
if __name__ == "__main__":
  ...
  parser.add_argument('--eipvalue', required=False, action='store')
